...
void UIViewPortManager::ET_update() {
    ...
    if(oldViewPort != viewPort) {
        LogDebug("[UIViewPortManager::ET_update] Set viewport: [%d x %d]", viewPort.x, viewPort.y);
...
void UIViewPortManager::ET_setViewPort(const Vec2i& newSize) {
    ...
    viewPort = newSize;
    LogDebug("[UIViewPortManager::ET_setViewPort] Set viewport: [%d x %d]", viewPort.x, viewPort.y);
